...
timeseries[:, 1] .= [0.21690008444818204, -0.26299612444316517, -0.2424012650991593]
step = RungeKutta4(3)
for i in ProgressBar(2:iterations)
    step(newton_leipnick, timeseries[:, i-1], dt)
    timeseries[:, i] .= step.xⁿ⁺¹ .+ ϵ * randn(3) * sqrt(dt)
end

state_space_partitions = StateSpacePartition(timeseries; method = Tree(; structured = false, arguments = 0.1))
